...
class Bank{
    ...
    async transfer(userId){
        ...
        const transfer = await prisma.transfer.create({data: {valor: pay, enviouId: find.id, recebeuId: findReceiver.id}})
    ...
    async consult(userId){
        ...
        return({message: `Olá, ${find.nome}`, saldo: saldo, success: true})
    };
